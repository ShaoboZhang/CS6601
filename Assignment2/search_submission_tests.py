...
from Assignment2.submission import *
from Assignment2.visualize_graph import plot_search
...
class TestBasicSearch(unittest.TestCase):
    ...
    def test_ucs(self):
        ...
        self.romania.reset_search()
        # path = bidirectional_a_star(self.romania, start, goal, euclidean_dist_heuristic)
        path = tridirectional_upgraded(self.romania, ['a','s','i'])
        self.draw_graph(self.romania, node_positions=node_positions, start=start, goal=goal, path=path)
    ...
    @staticmethod
    def draw_graph(graph, node_positions=None, start=None, goal=None, path=None):
...
class TestBidirectionalSearch(unittest.TestCase):
    ...
    def test_bidirectional_ucs(self):
        ...
        plot_search(self.atlanta, 'atlanta_search_bidir_ucs.json', path, all_explored)
    ...
    def test_bidirectional_a_star(self):
        ...
        all_explored = self.atlanta.explored_nodes
        plot_search(self.atlanta, 'atlanta_search_bidir_a_star.json', path, all_explored)
