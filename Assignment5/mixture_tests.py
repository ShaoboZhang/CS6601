from Assignment5.helper_functions import *
from Assignment5.submission import *
import numpy as np
import unittest
...
def generate_test_mixture(data_size, means, variances, mixing_coefficients):
    """
    Generate synthetic test data for a GMM based on fixed means, variances and mixing coefficients.
    params:
    data_size = (int)
    means = [float]
    variances = [float]
...
class K_means_test(unittest.TestCase):
    ...
    def test_initial_means(self, initial_means=get_initial_means):
    ...
    def test_k_means(self, k_means_cluster):
        """
        Testing your implementation of k-means on the segmented bird_color_24 reference images.
        """
        k_min = 2
        k_max = 6
        image_dir = 'images/'
...
class GMMTests(unittest.TestCase):
    ...
    def test_gmm_covariance(self, compute_sigma):
        """ Testing implementation of covariance matrix computation explicitly"""
        image_file = 'images/bird_color_24.png'
        image_matrix = image_to_matrix(image_file)
        image_matrix = image_matrix.reshape(-1, 3)
        m, n = image_matrix.shape
        ...
        self.assertTrue(np.allclose(SIGMA, sigma), msg="Incorrect covariance matrix.")
        print_success_message()

    def test_gmm_prob(self, prob):
        """
        Testing the GMM method for calculating the probability of a given point belonging to a component.
        """
        image_file = 'images/bird_color_24.png'
        image_matrix = image_to_matrix(image_file)
        image_matrix = image_matrix.reshape(-1, 3)
    ...
    def test_gmm_m_step(self, M_step):
        ...
        self.assertTrue(np.allclose(pi, expected_PI), msg="Incorrect new coefficient matrix.")
        self.assertTrue(np.allclose(mu, expected_MU), msg="Incorrect new means matrix.")
        self.assertTrue(np.allclose(sigma, expected_SIGMA), msg="Incorrect new covariance matrix.")
        print_success_message()

    def test_gmm_likelihood(self, likelihood):
        """Testing the GMM method for calculating the overall model probability.
        ...
        self.assertEqual(np.round(lkl), -23943.0, msg="Incorrect likelihood value returned")
        # expected_lkl =
        print_success_message()

    def test_gmm_train(self, train_model, likelihood):
        """Test the training procedure for GMM.
        returns: gmm = GaussianMixtureModel
        """
        image_file = 'images/bird_color_24.png'
        image_matrix = image_to_matrix(image_file)
        image_matrix = image_matrix.reshape(-1, 3)
        ...
        self.assertTrue(diff_check, msg=("Model likelihood increased by less than %d for a two-mean mixture" % likelihood_thresh))

        print_success_message()

    def test_gmm_segment(self, train_model, segment):
        """
        Apply the trained GMM to unsegmented image and generate a segmented image.
        returns: segmented_matrix = numpy.ndarray[numpy.ndarray[float]]
        """
        image_file = 'images/bird_color_24.png'
        image_matrix = image_to_matrix(image_file).reshape(-1, 3)
        num_components = 5

        MU, SIGMA, PI, r = train_model(image_matrix, num_components, convergence_function=default_convergence)

        segment = segment(image_matrix, MU, num_components, r)

        segment_num_components = len(np.unique(segment, axis=0))
        self.assertTrue(segment_num_components == r.shape[0], msg="Incorrect number of image segments produced")
        segment_sort = np.sort(np.unique(segment, axis=0), axis=0)
        mu_sort = np.sort(MU, axis=0)
        self.assertTrue((segment_sort == mu_sort).all(), msg="Incorrect segment values. Should be be MU values")
        print_success_message()

    def test_gmm_cluster(self, cluster):
        """
        Apply the trained GMM to unsegmented image and generate a clusters.
        returns: segmented_matrix = numpy.ndarray[numpy.ndarray[float]]
        """
    ...
    def test_gmm_best_segment(self, best_segment):
        """
        Calculate the best segment generated by the GMM and compare the subsequent likelihood of a reference segmentation.
        Note: this test will take a while to run.
        returns:
        ...
        num_components = 3
        iters = 10
        # generate best segment from 10 iterations and extract its likelihood
    ...
    def test_gmm_improvement(self, improved_initialization, initialize_parameters, train_model, likelihood):
        ...
        self.assertTrue(likelihood_diff >= diff_thresh,
                        msg=("Model likelihood less than %d higher than original model" % diff_thresh))
        print_success_message()
    ...
    def test_bayes_info(self, bayes_info_criterion):
        """
        Test for your implementation of BIC on fixed GMM values. Should be about 727045.
        returns: BIC = float
        """
        image_file = 'images/bird_color_24.png'
        image_matrix = image_to_matrix(image_file).reshape(-1, 3)
        ...
        self.assertTrue(np.isclose(48500, b_i_c, atol=500), msg="BIC calculation incorrect.")
        print_success_message()


    def test_bonus(self, bonus):
        n = 4
        num_points = 5000
        num_means = 1000
